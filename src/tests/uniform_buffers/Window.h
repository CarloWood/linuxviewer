...
void main()
{
  positions[0].x = TopPosition::x[27] - 1.0;
...
void main()
{
  positions[0].x = TopPosition::x[27];
  positions[1].y = LeftPosition::y;
...
else
{
  ...
  void draw_imgui() override final
  {
    ...
    if (m_top_buffer.is_ready())
      ((TopPosition*)(m_top_buffer[frame_index].pointer()))->x[top_position_array_index] = m_top_position + frame_index.get_value() * 0.1f;
