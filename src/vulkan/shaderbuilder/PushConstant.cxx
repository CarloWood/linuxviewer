...
std::string PushConstant::declaration(pipeline::Pipeline* pipeline) const
{
  std::size_t const hash = std::hash<std::string>{}(prefix());
  auto res = pipeline->glsl_id_str_to_declaration_context().try_emplace(prefix(), prefix(), hash);
  // This prefix should already have been inserted by Pipeline::add_push_constant.
  //
