...
VkResult HelloTriangleSwapChain::acquireNextImage(uint32_t* imageIndex)
{
...
VkResult HelloTriangleSwapChain::submitCommandBuffers(VkCommandBuffer const* buffers, uint32_t* imageIndex)
{
...
void HelloTriangleSwapChain::createSwapChain()
{
  ...
  if (vkCreateSwapchainKHR(device.device(), &createInfo, nullptr, &swapChain) != VK_SUCCESS) { throw std::runtime_error("failed to create swap chain!"); }
...
void HelloTriangleSwapChain::createImageViews()
{
  ...
  for (size_t i = 0; i < swapChainImages.size(); ++i)
  {
    ...
    if (vkCreateImageView(device.device(), &viewInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS) { throw std::runtime_error("failed to create texture image view!"); }
  }
}
...
void HelloTriangleSwapChain::createRenderPass()
{
  ...
  if (vkCreateRenderPass(device.device(), &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) { throw std::runtime_error("failed to create render pass!"); }
}
...
void HelloTriangleSwapChain::createFramebuffers()
{
  ...
  for (size_t i = 0; i < imageCount(); ++i)
  {
    ...
    if (vkCreateFramebuffer(device.device(), &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) { throw std::runtime_error("failed to create framebuffer!"); }
  }
}
...
void HelloTriangleSwapChain::createDepthResources()
{
  ...
  for (int i = 0; i < depthImages.size(); ++i)
  {
    ...
    if (vkCreateImageView(device.device(), &viewInfo, nullptr, &depthImageViews[i]) != VK_SUCCESS) { throw std::runtime_error("failed to create texture image view!"); }
  }
}
...
VkSurfaceFormatKHR HelloTriangleSwapChain::chooseSwapSurfaceFormat(std::vector<VkSurfaceFormatKHR> const& availableFormats)
{
...
VkPresentModeKHR HelloTriangleSwapChain::chooseSwapPresentMode(std::vector<VkPresentModeKHR> const& availablePresentModes)
{
...
VkExtent2D HelloTriangleSwapChain::chooseSwapExtent(VkSurfaceCapabilitiesKHR const& capabilities)
{
