...
class ShaderInputData
{
  ...
  struct PushConstantRangeCompare {
    bool operator()(vk::PushConstantRange const& pcr1, vk::PushConstantRange const& pcr2) const
    {
      ...
      // If we return "equal" here, then the old range will be replaced, and the way that the ranges
      // are generated by ... that should be the smaller one. For example:
 ...
 public:
  ...
  shaderbuilder::VertexAttributeDeclarationContext& vertex_shader_location_context() { return m_vertex_shader_location_context; }
    ...
    MemberIndex, MaxAlignment, Offset, GlslIdStr> const& member_layout)
{
  ...
  // Get a reference to the associated DeclarationContext object, creating one if necessary.
  glsl_id_str_to_declaration_context_container_t::iterator declaration_context_iter = glsl_id_str_to_declaration_context().find(push_constant.prefix());
  if (declaration_context_iter == glsl_id_str_to_declaration_context().end())
  {
    auto res2 = glsl_id_str_to_declaration_context().try_emplace(push_constant.prefix(), std::make_unique<shaderbuilder::PushConstantDeclarationContext>(push_constant.prefix(), hash));
    // We just used find() and it wasn't there?!
    ASSERT(res2.second);
    declaration_context_iter = res2.first;
  }
}
    ...
    MemberIndex, MaxAlignment, Offset, GlslIdStr> const& member_layout)
{
  ...
  if (declaration_context_iter == glsl_id_str_to_declaration_context().end())
  {
    auto res2 = glsl_id_str_to_declaration_context().try_emplace(push_constant.prefix(), std::make_unique<shaderbuilder::DeclarationContext>(push_constant.prefix(), hash));
    // We just used find() and it wasn't there?!
    ASSERT(res2.second);
    declaration_context_iter = res2.first;
  }
