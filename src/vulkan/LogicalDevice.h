...
class LogicalDevice
{
 ...
 public:
  ...
  vk::PipelineLayout realize_pipeline_layout(
      sorted_descriptor_set_layouts_t::wat const& realized_descriptor_set_layouts_w,    // wat because the binding numbers might be adjusted.
    ...
    uint32_t array_element_count, FrameResourceIndex number_of_frame_resources) const
{
  ...
  ASSERT(frame_resources == 1 ||
      (frame_resources == number_of_frame_resources.get_value() &&
       ...
       // Note: it can't happen that this descriptor_set was created with no frame resources
       //       when now we're trying to add -say- a uniform buffer (which is a frame resource)
       //       because descriptor sets are allocated when all of their descriptors are
       //       already known.
